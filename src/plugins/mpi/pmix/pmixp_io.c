...
inline static int _rcvd_swithch_to_body(pmixp_io_engine_t * eng)
{
	...
	if (eng->header.unpack_hdr_cb) {
		...
		if ((rc =
		     eng->header.unpack_hdr_cb(eng->rcvd_hdr,
					       eng->rcvd_hdr_host))) {
...
{
	return eng->rcvd_padding
	    && eng->rcvd_pad_recvd < eng->rcvd_padding;
}
...
bool pmix_io_send_pending(pmixp_io_engine_t * eng)
{
	...
	if (eng->send_current == NULL) {
		...
		if ((rc = _send_set_current(eng, msg))) {
			PMIXP_ERROR_NO(rc,
				       "Cannot switch to the next message");
...
void pmix_io_send_progress(pmixp_io_engine_t * eng)
{
	...
	while (pmix_io_send_pending(eng)) {
		...
		if (!_send_header_ok(eng)) {
			...
			int cnt =
			    pmixp_write_buf(fd, offs, remain, &shutdown,
					    false);
			if (shutdown) {
				pmix_io_finalize(eng, shutdown);
				return;
